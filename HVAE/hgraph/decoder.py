...
class HierMPNDecoder(nn.Module):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, latent_size, depthT, depthG, dropout, attention=False):
        ...
        self.rnn_cell = self.hmpn.tree_encoder.rnn #为了和encoder是同一个结构，所以直接导入encoder的rnn类型。
        self.E_assm = self.hmpn.E_i #encoder里面的embedding 
    ...
    def init_decoder_state(self, tree_batch, tree_tensors, src_root_vecs):
        ...
        num_mess = len(tree_tensors[1]) #tree_tensors[1] = fmess
        agraph = tree_tensors[2].clone()
        bgraph = tree_tensors[3].clone()

        for i,tup in enumerate(tree_tensors[-1]): #Tree_tensors[-1] 
            root = tup[0]
        ...
        new_tree_tensors = tree_tensors[:2] + [agraph, bgraph] + tree_tensors[4:] #对agraph, bgraph进行了一个更新
        htree = HTuple() #是一个类，可以向里面加node, mess, vmask, emask等信息
        htree.mess = self.rnn_cell.get_init_state(tree_tensors[1], src_root_vecs) #在rnn.py文件内的get_init_state函数，返回指定大小的0值矩阵。
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        src_root_vecs, src_tree_vecs, src_graph_vecs = src_mol_vecs #src_mol_vecs是编码器的输出经过正则化后的embedding
        init_vecs = src_root_vecs if self.latent_size == self.hidden_size else self.W_root(src_root_vecs) #如果latent_size不等于laten_size，就进行维度的转换
        ...
        for i in range(batch_size):
            root = tree_batch.nodes[ tree_scope[i][0] ] #tree_scope[i][0]等于offset，偏移量；root应该是一个原子
            clab, ilab = self.vocab[ root['label'] ]  #根据原子的label，这里是clusters的编号，返回对应的clusters和有连接处的clusters
            all_cls_preds.append( (init_vecs[i], i, clab, ilab) ) #cluster prediction
            new_atoms.extend(root['cluster'])

        subgraph = self.update_graph_mask(graph_batch, new_atoms, hgraph) #返回两个数， new_atom_index, new_bond_index
        graph_tensors = self.hmpn.embed_graph(graph_tensors) + (graph_tensors[-1],) #preprocess graph tensors

        maxt = max([len(x) for x in orders])
        max_cls_size = max( [len(attr) * 2 for node,attr in tree_batch.nodes(data='cluster')] ) #为什么要乘2
        ...
        topo_scores = self.get_topo_score(src_tree_vecs, batch_idx, topo_vecs) #这里应该是正式开始预测，输入的是encoder的输出与现在已存在分子结构的embedding值
