...
def get_mol(smiles):
    ...
    if mol is not None: Chem.Kekulize(mol) #将芳香环转换为单键和双键
    return mol
...
def copy_edit_mol(mol): #复制一个可编辑的mol
    new_mol = Chem.RWMol(Chem.MolFromSmiles('')) 
    #复杂的操作可以使用rdkit.Chem.rdchem.RWMol类（用于分子读写的类）。这个类在修改分子方面，性能更好，它可以提供一个“活动的”分子，并且共享了mol对象的操作接口。修改完毕后，只需要用GetMol()就可以获得最终的分子
    for atom in mol.GetAtoms():
...
def get_clique_mol(mol, atoms): #clique：派系；私党；小圈子
    smiles = Chem.MolFragmentToSmiles(mol, atoms, kekuleSmiles=True) #返回分子片段的smiles，这里是指mol分子中atoms的片段
    new_mol = Chem.MolFromSmiles(smiles, sanitize=False)
    new_mol = copy_edit_mol(new_mol).GetMol()#复制一个同new_mol一样的分子。
    new_mol = sanitize(new_mol) #检查一个新分子是否有效
...
def get_inter_label(mol, atoms, inter_atoms):
    ...
    if new_mol.GetNumBonds() == 0: #如果新mol内没有键
        inter_atom = list(inter_atoms)[0]
        for a in new_mol.GetAtoms():
            a.SetAtomMapNum(0) #给new_mol中的原子全部打上0的标签
