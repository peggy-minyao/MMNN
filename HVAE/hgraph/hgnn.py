...
def make_cuda(tensors): #转换为tensors，并传入到cuda内？
    tree_tensors, graph_tensors = tensors
    make_tensor = lambda x: x if type(x) is torch.Tensor else torch.tensor(x) #如果输入的x不是tensor则转换为tensor
...
class HierVAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.R_mean = nn.Linear(args.hidden_size, args.latent_size) #latent_size = 32，hidden_size =25
        self.R_var = nn.Linear(args.hidden_size, args.latent_size)

    def rsample(self, z_vecs, W_mean, W_var, perturb=True): #应该是对编码器生成的z-dimension正则化的过程（均值和方差）
        batch_size = z_vecs.size(0)
        z_mean = W_mean(z_vecs) #通过一个神经网络，计算这个隐向量的正态分布的均值
        z_log_var = -torch.abs( W_var(z_vecs) ) #通过一个神经网络，计算这个隐向量的正态分布的方差，再加一个绝对值和负号，保证他是负数
        kl_loss = -0.5 * torch.sum(1.0 + z_log_var - z_mean * z_mean - torch.exp(z_log_var)) / batch_size 
        #kl_loss返回的是该隐向量的专属正态分布与标准真该分布的差异，torch.exp返回input的e为底数，input为幂的结果
        epsilon = torch.randn_like(z_mean).cuda() #应该是随机选取了一个高斯噪音
        z_vecs = z_mean + torch.exp(z_log_var / 2) * epsilon if perturb else z_mean
        return z_vecs, kl_loss

    def sample(self, batch_size, greedy):
        root_vecs = torch.randn(batch_size, self.latent_size).cuda() #采样，随机生成一组数，根据这组数进行分子的生成。
    ...
    def forward(self, graphs, tensors, orders, beta, perturb_z=True):
        ...
        root_vecs, tree_vecs, _, graph_vecs = self.encoder(tree_tensors, graph_tensors) #hnode, hmess, agraph, bgraph
...
class DirectVAE(nn.Module): #定向生成模型结构
    ...
    def properties(self,mols):
        properties = 0
        for mol in mols:
            propertie = Chem.QED.qed(mol)
            properties = properties + propertie
        propertie_value = properties/len(mols)
    ...
    def forward(self, graphs, tensors, orders, beta, perturb_z=True):
        ...
        loss_1, wacc, iacc, tacc, sacc = self.decoder((root_vecs, root_vecs, root_vecs), graphs, tensors, orders)
        mols = decoder.decode((root_vecs, root_vecs, root_vecs), greedy=True, max_decode_step=150) #解码一个？分子出来（可能是一个batch的分子）
        loss_2 = properties(mols)-1
        loss = loss_1 + loss_1
        return loss + beta * kl_div, kl_div.item(), wacc, iacc, tacc, sacc
